...
class ObjaverseData(Dataset):
    ...
    def get_T(self, target_xyz, cond_xyz): # rotation and translation
        # (3, 4)
        # R, T = target_RT[:3, :3], target_RT[:, -1]
        # T_target = -R.T @ T # position of the target camera w.r.t. the object

        # R, T = cond_RT[:3, :3], cond_RT[:, -1]
        # T_cond = -R.T @ T # position of the conditional camera w.r.t. the object

        # theta_cond, azimuth_cond, z_cond = self.cartesian_to_spherical(T_cond[None, :]) 
        # theta_target, azimuth_target, z_target = self.cartesian_to_spherical(T_target[None, :])

        theta_cond, azimuth_cond, z_cond = self.cartesian_to_spherical(cond_xyz) 
        theta_target, azimuth_target, z_target = self.cartesian_to_spherical(target_xyz)
        
        d_theta = theta_target - theta_cond
        d_azimuth = (azimuth_target - azimuth_cond) % (2 * math.pi)
        d_z = z_target - z_cond
        
        d_T = torch.tensor([d_theta.item(), math.sin(d_azimuth.item()), math.cos(d_azimuth.item())])
    ...
    def __getitem__(self, index):
        ...
        # clip_embedding = CLIP(cond_im) # (768,)
        # cond_embedding = torch.concat([clip_embedding, data["T"]]) # (772)
        # condition = feedforward(cond_embedding) # (768, )
        # img_prediction = Unet(t_0, gaussian noise, condition) # (h, w, 3)

        # img_ground_truth = data["image_target"] # (h, w, 3)
